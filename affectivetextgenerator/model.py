...
def perturb_past(
    ...
    past = torch.stack(list([torch.stack(list(p), dim=0) for p in past]), dim=0)
    grad_accumulator = [
    ...
    for i in range(num_iterations):
        ...
        res = model(last, past_key_values=perturbed_past)
        all_logits = res[0]
        all_hidden = res[2]
        hidden = all_hidden[-1]
...
def full_text_generation(
    ...
    unpert_gen_tok_text = torch.tensor([tokenizer.encode("Hello I am testing this string")])
    ...
    for i in range_func:
        ...
        if past is None and output_so_far is not None:
            ...
            if output_so_far.shape[1] > 1:
                res = model(output_so_far[:, :-1])
                past = res[1]

        res = model(output_so_far)
        unpert_logits = res[0]
        unpert_past = res[1]
        unpert_all_hidden =res[2]
        ...
        res = model(last, past_key_values=pert_past)
        pert_logits = res[0]
        past = res[1]
        pert_all_hidden = res[2]
